...
async def run_predictions(
    ...
    if not profile or not assessment:
        raise ValueError("Incomplete background data profiles setup to evaluate predictions.")
    ...
    for disease in target_diseases:
        ...
        try:
            ...
            errors.append({"disease": disease, "stage": "feature_engineering", "error": str(exc)})
            continue
        ...
        try:
            ...
            if scaler and disease in ("cvd", "stroke", "chd"):
                orders = {"cvd": CVD_FEATURE_ORDER, "stroke": STROKE_FEATURE_ORDER, "chd": CHD_FEATURE_ORDER}
                df_raw = pd.DataFrame([features], columns=orders[disease])
                feature_input = pd.DataFrame(scaler.transform(df_raw), columns=orders[disease])
            elif disease == "hyp":
        ...
        try:
            ...
            errors.append({"disease": disease, "stage": "persistence", "error": str(exc)})
