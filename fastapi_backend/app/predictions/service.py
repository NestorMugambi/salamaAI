...
from .feature_engineering.cvd import engineer_cvd_features, EXPECTED_FEATURES as CVD_FEATURE_ORDER
from .feature_engineering.hypertension import engineer_hyp_features
from .feature_engineering.chd import engineer_chd_features
from .feature_engineering.stroke import engineer_stroke_features, EXPECTED_FEATURES as STROKE_FEATURE_ORDER
...
_MODEL_PATHS: dict[str, tuple[str, str]] = {
    ...
    "hyp":    ("xgbhypertension.joblib", "xgbhyp_v1"),
    "chd":    ("chdriskv1.joblib",        "chdrisk_v1"),
    "stroke": ("xgbstrokev5.joblib",     "xgbstroke_v5"),  # UPDATED: Utilizing new XGBoost v4 model
}
...
def _load_scaler(disease: str) -> Any | None:
    ...
    if disease not in _loaded_scalers:
        ...
        logger.info("Loading scaler execution configuration for %s from %s", disease, path)
        if not path.exists():
            raise FileNotFoundError(
...
async def _fetch_assessment(user_id: UUID, session: AsyncSession) -> HealthAssessment | None:
    result = await session.execute(
...
async def run_predictions(
    ...
    for disease in target_diseases:
        if disease not in _MODEL_PATHS:
            errors.append({"disease": disease, "stage": "validation", "error": "Unknown disease key"})
            continue
        ...
        try:
            ...
            logger.error("Feature engineering failed for '%s': %s\n%s", disease, exc, tb)
            errors.append({
                "disease":   disease,
        ...
        try:
            ...
            tb = traceback.format_exc()
            logger.error("Artifact payload context load failed for '%s': %s\n%s", disease, exc, tb)
            errors.append({
                "disease":   disease,
                "stage":     "model_load",
                "error":     str(exc),
        ...
        try:
            # UPDATED: Added isolation framework path support for both cvd and stroke scaled targets
            if scaler is not None and disease in ("cvd", "stroke"):
                feature_order = CVD_FEATURE_ORDER if disease == "cvd" else STROKE_FEATURE_ORDER
                ...
                # Fallback path for standard models not requiring normalization arrays
        ...
        try:
            ...
            results.append(DiseasePredictionResult(
                disease=disease,
            ...
            session.add(RiskAssessmentResult(
                user_id=user_id,
                disease=disease,
