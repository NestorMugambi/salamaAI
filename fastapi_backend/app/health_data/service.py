...
class GenericService(Generic[T, S]):
    ...
    async def create_record(self, user_id: str, data: S, **kwargs) -> T:
    ...
    async def get_record(self, record_id: int, user_id: str) -> Optional[T]:
    ...
    async def get_user_records(self, user_id: str, limit: int = 100) -> List[T]:
        """Fetch all records for a user."""
        result = await self.session.execute(
            select(self.model).where(self.model.user_id == user_id).limit(limit)
        )
        return result.scalars().all()

    async def get_all_records(self, limit: int = 100) -> List[T]:
